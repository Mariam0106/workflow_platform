@props(['name', 'value' => null, 'users', 'entities', 'departments', 'label' => 'Utilisateur', 'hint' => null, 'syncBusinessFunctionField' => null])
...
<div data-uid="{{ $uid }}" @if ($syncBusinessFunctionField) data-sync-business-function="{{ $syncBusinessFunctionField }}" @endif>
    <label class="mb-1.5 block text-[13px] font-medium text-slate-700">{{ $label }}</label>
    ...
    <div class="mb-2 grid grid-cols-2 gap-2">
        ...
        <select data-role="department-filter" class="rounded-lg border border-brand-border bg-white px-2.5 py-2 text-[13px] text-brand-navy shadow-sm focus:border-brand-blue focus:outline-none focus:ring-4 focus:ring-brand-blue/10">
            ...
            @foreach ($departments as $department)
                <option value="{{ $department->id }}" data-entity="{{ $department->entity_id }}">{{ $department->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="relative">
        ...
        <input type="hidden" name="{{ $name }}" data-role="value" value="{{ $value }}">
    ...
    <script type="application/json" data-role="dataset">@json($usersPayload)</script>
    ...
    <script>
        (function () {
            ...
            function currentEntityId() {
                return {{ $selectedUser?->entity_id ?? 'null' }};
            }
            function currentDepartmentId() {
                return {{ $selectedUser?->department_id ?? 'null' }};
            }
