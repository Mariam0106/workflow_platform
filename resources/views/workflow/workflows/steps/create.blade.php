@extends('layouts.admin', ['title' => 'Nouvelle étape'])

@section('content')
    <x-page-header title="Nouvelle étape" :description="'Workflow « ' . $workflow->name . ' »'">
        <x-slot:actions>
            <x-button href="{{ route('workflow.admin.workflows.edit', $workflow) }}" variant="secondary" icon="arrow-left">Retour</x-button>
        </x-slot:actions>
    </x-page-header>

    <form method="POST" action="{{ route('workflow.admin.workflows.steps.store', $workflow) }}" class="max-w-xl space-y-4">
        @csrf
        <x-card>
            <div class="space-y-4">
                <x-form-input name="name" label="Nom" required hint="Ex. « Validation Finance »." />
                <x-form-input name="code" label="Code" required hint="Identifiant unique dans ce workflow, ex. « FINANCE_APPROVAL »." />
                ...
                <div>
                    ...
                    <select id="validator_type" name="validator_type" required onchange="updateValidatorFields()"
                        ...
                        <option value="BUSINESS_FUNCTION" @selected(old('validator_type') === 'BUSINESS_FUNCTION')>Fonction Métier</option>
                        <option value="USER" @selected(old('validator_type') === 'USER')>Utilisateur désigné</option>
                        <option value="N_PLUS_1" @selected(old('validator_type') === 'N_PLUS_1')>Responsable direct (N+1) du demandeur</option>
                        <option value="ENTITY_MANAGER" @selected(old('validator_type') === 'ENTITY_MANAGER')>Responsable de l'Entité du demandeur</option>
                        <option value="DEPARTMENT_MANAGER" @selected(old('validator_type') === 'DEPARTMENT_MANAGER')>Responsable du Département du demandeur</option>
                    </select>
                ...
                <div id="field-business-function">
                    <x-form-select name="validator_reference_business_function" label="Fonction Métier validatrice" :options="$businessFunctions" placeholder="—"
                                    hint="Toute personne occupant cette fonction pourra valider cette étape." />
                </div>
                ...
                <x-form-checkbox name="is_end" label="Étape de fin" hint="Un workflow doit avoir au moins une étape de fin." />
            </div>
        </x-card>
    ...
    <script>
        ...
        function updateValidatorFields() {
            ...
            // Le filtre Entité/Département de l'Utilisateur validateur sert
            // aussi de simple aperçu ("qui a cette Fonction Métier ?") -
            // utile même si le Type reste "Fonction Métier" (pas de
            // personne précise à soumettre dans ce cas).
