...
class UserRepository implements UserRepositoryInterface
{
    ...
    {
        // BR-06 : "avec ce Role" doit couvrir tout Utilisateur AUTORISE
        // pour ce Role (table pivot user_application_roles), pas
        // seulement ceux pour qui il s'agit du Role par defaut - sinon
        // un Utilisateur multi-role habilite Validateur mais dont le
        // Role par defaut est User serait invisible ici.
        return $this->model->newQuery()
            ->where('is_active', true)
            ->whereHas('applicationRoles', function ($query) use ($role): void {
    ...
    public function createFromData(CreateUserData $dto): User
    {
        ...
        // BR-06 : synchronise les Roles Applicatifs AUTORISES sur le
        // pivot - toArray() ne transporte que la colonne
        // default_application_role_id, jamais la liste complete.
        $user->applicationRoles()->sync($dto->applicationRoleIds);
    ...
    public function updateFromData(int $id, UpdateUserData $dto): User
    {
        ...
        // BR-06 : ne resynchronise le pivot que si l'appelant a
        // effectivement fourni une nouvelle liste de Roles - sinon un
        // update partiel portant uniquement sur, par ex., le
        // department_id supprimerait silencieusement tous les autres
        // Roles autorises de l'Utilisateur.
        if ($dto->hasApplicationRoleIds()) {
            $user->applicationRoles()->sync($dto->applicationRoleIds);
        }
