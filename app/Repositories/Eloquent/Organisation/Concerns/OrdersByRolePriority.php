...
trait OrdersByRolePriority
{
     ...
     * calling this.
     *
     * NOTE (multi-role, BR-06) : ranks on the User's DEFAULT Role
     * (`users.default_application_role_id`), not the full authorized
     * set - this fallback resolves "the" manager for a department/
     * entity, a single answer is expected, so it deliberately uses the
     * single, stable default Role rather than picking one arbitrarily
     * out of a multi-role User's several authorized Roles.
     */
    private function orderByRolePriority(Builder $query): Builder
    {
        ...
        return $query
            ->join('application_roles', 'users.default_application_role_id', '=', 'application_roles.id')
            ->select('users.*')
