...
final readonly class UpdateUserData
{
    ...
    public function __construct(
        ...
        /** @var list<int>|null every authorized Application Role id (BR-06) */
        public ?array $applicationRoleIds = null,
        public ?int $defaultApplicationRoleId = null,
    ...
    public static function fromArray(array $data): self
    {
        return new self(
            ...
            applicationRoleIds: array_key_exists('application_role_ids', $data)
                ? array_values(array_unique(array_map(static fn ($id): int => (int) $id, $data['application_role_ids'])))
                : null,
            defaultApplicationRoleId: array_key_exists('default_application_role_id', $data) ? (int) $data['default_application_role_id'] : null,
            managerId: array_key_exists('manager_id', $data) ? (blank($data['manager_id']) ? null : (int) $data['manager_id']) : null,
    ...
    public function toArray(): array
    {
        ...
        $sourceKeys = [
            'entity_id', 'department_id', 'business_function_id', 'default_application_role_id',
    ...
    /**
     * Whether the caller submitted a new set of authorized Application
     * Roles (BR-06) - distinct from `toArray()` since this is synced
     * onto the user_application_roles pivot, not written as a column.
     */
    public function hasApplicationRoleIds(): bool
    {
        return array_key_exists('application_role_ids', $this->provided);
    }

    public function isEmpty(): bool
    {
        return $this->toArray() === [] && ! $this->hasApplicationRoleIds();
    }
}