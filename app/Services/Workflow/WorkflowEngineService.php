...
class WorkflowEngineService implements WorkflowEngineInterface
{
    ...
    /**
     * BR-53 (par extension) : au plus un Brouillon par couple
     * Utilisateur/Formulaire - un appel répété (autosave périodique
     * côté navigateur) met à jour le même Brouillon plutôt que d'en
     * créer un nouveau à chaque fois. Contrairement à submit(), tolère
     * un tableau "values" vide ou partiel (SaveDraftData, contrairement
     * à SubmitRequestData, n'a pas cette contrainte).
     *
     * Assigné à l'Étape de début du Workflow dès sa création (comme un
     * Request soumis) uniquement parce que `current_step_id` est une
     * colonne NOT NULL en base (voir migration create_requests_table) -
     * ça n'a aucune signification métier tant que le statut reste
     * Draft : un Brouillon n'est jamais réellement "à cette étape" pour
     * un Validateur (BR-36 filtre déjà sur status=Submitted, donc un
     * Brouillon n'apparaît jamais dans une file de validation).
     *
     * @throws FormNotPublishedException si le Form n'existe pas ou n'est pas publié.
     */
    public function saveDraft(SaveDraftData $data): Request
    {
        return DB::transaction(function () use ($data) {
            ...
            if ($draft === null) {
                ...
                // touch() à défaut d'un vrai changement de colonne :
                // c'est updated_at qui sert de "dernière sauvegarde à"
                // côté écran (voir MyRequestController::saveDraft()).
                $draft->touch();
            }

            foreach ($data->values as $formFieldId => $value) {
                RequestValue::updateOrCreate(
                    ['request_id' => $draft->id, 'form_field_id' => $formFieldId],
                    ['value' => $value],
                );
            }

            return $draft->fresh(['requestValues']);
        });
    }
     ...
     * avance la Request, et la marque Completed si le Step VALIDÉ
     * (celui qu'on vient d'approuver, pas celui qu'on atteint) est une
     * Étape de fin.
     *
     * CORRECTION : la version précédente vérifiait `is_end` sur
     * $nextStep (le Step qu'on ATTEINT après la Transition) - une
     * Demande passant Step1 (N+1) -> Step2 (Utilisateur désigné,
     * is_end=true) se retrouvait donc marquée Terminée dès l'entrée
     * dans Step2, SANS jamais attendre la validation de Step2
     * lui-même. Chaque Étape configurée avec un Validateur (BR-06 sur
     * WorkflowStep) doit recevoir sa PROPRE validation avant que la
     * Demande n'avance ou ne se termine - une Étape de fin doit
     * recevoir SA validation, pas seulement être "atteinte", avant de
     * clore le Workflow. Corrigé pour vérifier `is_end` sur $step (le
     * Step qu'on vient de valider) : s'il est de fin, la Demande se
     * termine ICI, sans chercher de Transition sortante (BR-34 : une
     * Étape de fin n'en a jamais besoin) ; sinon, une Transition
     * sortante est requise (BR-22, WorkflowTransitionSelector lève
     * toujours une exception s'il n'y en a aucune - comportement
     * inchangé, une vraie erreur de configuration reste détectée).
     *
