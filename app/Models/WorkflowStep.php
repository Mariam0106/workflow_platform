...
class WorkflowStep extends Model
{
    ...
    /**
     * Nom lisible de la Référence du Validateur (Rôle, Fonction Métier
     * ou Utilisateur désigné) - null pour les 3 autres types, qui
     * n'ont pas de référence (voir ValidatorType). Utilisé dans le
     * récapitulatif d'étape plutôt que d'afficher juste le type.
     */
    public function validatorReferenceLabel(): ?string
    {
        return match ($this->validator_type) {
            ValidatorType::Role => ApplicationRole::find($this->validator_reference)?->name,
            ValidatorType::BusinessFunction => BusinessFunction::find($this->validator_reference)?->name,
            ValidatorType::User => User::find($this->validator_reference)?->full_name,
            default => null,
        };
    }
